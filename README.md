...
一种旧的方法、旧的技术、旧的计算机系统或应用程序，还有诸如没有人知道业务的历史遗留代码。产生原因：

- 几乎无法维护，难以修改。(如对业务不了解，改动引起变化太大)
- 代码遗失、子模块无法使用。(如第三方模块过时或无法使用)
- 没有文档或者过于简单。
- 代码逻辑不清晰。

1. 遗留代码——维护成本是否比开发成本高很多

1、交付压力：没有测试、充满BUG、赶工同时修BUG。
2、技能不足：编写测试能力不足，测试不完全。
3、功能复杂：多层嵌套、长的函数等代码问题。

2. 造成遗留系统的因素

多数系统不存在遗留代码导致系统遗留的问题——因为多数应用生命周期没有那么长。在近几年发展中，影响系统的主因变成了技术栈和可扩展性。可扩展性体现在软件的架构：如何支持更多的访问用户。可扩展性原因：

技术上：

- 纵向扩展：更好的机器提高并发数。
- 横向扩展：使用更多的机器提高并发数。(需引入“锁”机制)

组织结构上：

- 人员流动：理想团队，1老带1新，一倍成员。
- 知识分享：帮助新人成长，旧的知识得以传承。
...
当我们想要修改的代码都有测试覆盖的时候，才考虑对代码进行重构。

**更新技术栈**：

首先保证你的代码具有版本管理的功能，要优先保证本地的修改都是可以恢复的。做好上面实践。之后：
...
使用的库、依赖、模块有新版本出来，但我们不会立即升级这个软件的版本，这就是技术债务。还有使用的技术栈比较老旧，代码没有测试、测试覆盖率不够等。由于时间关系，并未做好。平时记得记录，必要的时候偿还，可避免应用成为遗留系统。
...
函数是指一段在一起的、可以做某一件事的程序。一段函数只做一件事——Tasking应用，将一个复杂的过程一步步地分解成一个个函数，每个函数只做它的名称对应的事。

3. 减少重复代码

主因：复制/粘贴，界定我们是否需要提取相同部分的原则是：事不过三。频率到达三次，就需要考虑重构。
...
编辑器/IDE工具重构，大部分开发工具都有相应的重构插件可以使用。重构技能中下列几个比较常用：
